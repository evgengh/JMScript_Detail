            ...
            if len(tmpLst) != 0:
                for itm in tmpLst:
                    if (self._checkElmTypeClls_(itm, "Controller")):
                        ...
                        tmpVar = prop.text
                        tmpVarNew = self.setPrefTrailInUniqNames["pref"] + prop.text
                        prop.text = tmpVarNew
    ...
    def _xElmUniqueName_(self, revCntrLst, parntNdClass, smplrsThru = False, thGrIfSmplr = None):
        ...
        while(len(tmpLst)!=0):
            ...
            while(itmCnt>0):
                ...
                tmpVar = prop.text
                tmpVarNew = prop.text + self.setPrefTrailInUniqNames["trail"] + str(itmCnt)
                prop.text = tmpVarNew
...
### Здесь и далее до аналогичного комментария - попытка переписать метода под XML, названия попплывут для удобства
    ...
    def extrHTTPDataNamesAndLinks(self):
        ...
        for j in allSmplElms:
            ...
            tmpLinkLst = [self._curLinkDict_[p].append((prntThGrNm, (prntSmplrNm, None, 0))) for p in self._curLinkList_ if p==j.find(xSet_1[0]).text]
            jArgsLst = [l for l in j.find(xSet_2[0]).find(xSet_2[0]).find(xSet_3[0]).findall(xSet_4[0])]
            for z in jArgsLst:
                tmpLst=[self._curDict_[k].append((prntThGrNm, (prntSmplrNm, z.find(xSet_6[0]).text, 0))) for k in self._curList_ if k == z.find(xSet_5[0]).text]
