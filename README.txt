...
Что нового в этой версии?!
    Добавлена возможность получения статистики по изменяемости (волатильности) параметров. См п. 9.1.
...
Недавние доработки и пояснения:
    1. Данная версия - рабочая бета-версия для стандартных форматов jmx-файлов (hashTree, в старой версии используется node). 
    ...
    2. Добавлены параметры настройки префиксов и разделителя. См. п.1.
    3. Добавлена скозная нумерация сэмплеров в разрезе контроллера, тредгруппы, тестплана.*
    4. Добавлена возможность усечения названий сэмплеров. Т.е. если название произведено от ссылки, то отбросится вся часть
    до крайнего правого "/" (включительно).*
    5. Появилась возможность при восстановлении оригинальных названий сэмплеров сохранить преобразованные в выходном JMX-файле.*
    ...
    6. Замечена особенность, что на 14" ноутбуке windows 10 немного свободно интерпретирует масштаб отрисовки окон и элементов приложения, 
